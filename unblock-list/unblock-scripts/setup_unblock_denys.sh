...
#2#wget -q -O /etc/storage/unblock/setup_unblock.sh https://raw.githubusercontent.com/Maximys717/greentea/master/unblock-list/unblock-scripts/setup_unblock_denys.sh
...
### 7. cron. uses user's name. Administration - Services. Turn ON 'Services Cron (scheduler)'. denys is used
#
echo 'adding task in crontab. User: denys'
#
echo '00 06 * * * denys /etc/storage/unblock/unblock_ipset.sh' >> /etc/storage/cron/crontabs/denys
echo 'task is added'
