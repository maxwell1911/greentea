...
### 7. cron. uses user's name. Administration - Services. Turn ON 'Services Cron (scheduler)'. nastia is used
#
echo 'adding task in crontab. User: nastia'
#
echo '00 06 * * * nastia /etc/storage/unblock/unblock_ipset.sh' >> /etc/storage/cron/crontabs/nastia
echo 'task is added'
